...
disp('SIMULATED ANNEALING FOR COURSE TIMETABLING :SCHOOL OF EDUCATION');disp(' ');
...
Tmax=T;
warning off
disp('SIMULATED ANNEALING FOR COURSE TIMETABLING');disp(' ');
...
while T>Temp_end 
    ...
    for t=1:Lk 
        ...
        e = randi(1,1,[1 24]); % random event (course)
        s = randi(1,1,[1 60]); % random time slot
        r = randi(1,1,[1 19]); % random venue
...
exam_time={'8am-9am','9am-10am','10am-11am','11am-12noon','12noon-1pm','1pm-2pm',...
...
%-------------------------------------------------------------------------
fprintf('%s      %s        %s        %s        %s       %s\n','Timeslot','                Course','Venue ','                  Capacity','# of Students','Examination Time');
fprintf('%s      %s        %s        %s        %s       %s\n','========','                ======','======','                  ========', '============','============');
...
%     end
    ...
    fprintf(' EXAM DAY %d ',round(i/14)+1); 
%     
    %%
    if st_pc{i}>(cap{i})
    for avv=1:19;
    ...
    %%
    fprintf('%s             %s      %s           %s             %s            %s\n',P{i},crs{i},vn{i},num2str(cap{i}),num2str(st_pc{i}),exam_time{i});%,num2str(teachers{i}));
    %fprintf('%s              ',num2str(teachers{i})); 

end

disp('**********The Cost Functions For SCHOOL OF EDUCATION*************')
disp(['The  Initial Cost Function: ',num2str(costIni)])
disp(['The Difference Cost Function: ',num2str(costDiff)])
disp(['The New Cost Function: ',num2str(costNew)])
disp(['The Best Cost Function: ',num2str(BestCost)])
snapnow;
d=toc ;
disp(['Simulation Time For SCHOOL OF EDUCATION: ',num2str(d)])

pause(2)
tic
sa_main_comm;
d1=toc;
disp(['Simulation Time For MASS COMM & POLITICAL SCIENCE: ',num2str(d1)])

%disp(['The Cost Function: ',num2str(costIni)])
snapnow;
pause(2);
...
sa_main_law
d2=toc;
disp(['Simulation Time For SCHOOL OF LAW: ',num2str(d2)])

%disp(['The Cost Function: ',num2str(costIni)])
snapnow;
Q=d + d1 + d2;

disp(['TOTAL OF SIMULATION TIME: ',num2str(Q)])
